...
| 职位雷达 | 内置 50+ 家校招官网来源，按求职方向（互联网与商科、考公 / 选调、国企央企与银行、法学、教师、医疗）筛选；一键监控并刷新。飞书招聘、北森、腾讯、快手、网易（含雷火）、米哈游、蚂蚁、TikTok 等招聘系统由本机后端直接读取岗位与 JD（无需插件，数百个岗位约半分钟），其余官网由 Chrome 插件读取。 |
...
| 投递进度 | 卡片、表格、动态三种视图；可导出 Excel（投递记录 + 进度时间线）；每一轮标记「通过 / 未通过 / 待定」，阶段自动推进；投递时冻结当时使用的简历版本，便于事后对照。 |
